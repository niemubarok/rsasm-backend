...
export default class RegistrasiController {
  public async pasienLama({ request, response }: HttpContextContract) {
    ...
    const getMaxAntrian = Database.from('booking_registrasi').where('tanggal_periksa', tglPeriksa).where('kd_dokter', kodeDokter).where('kd_poli', kodePoli).max('no_reg as no_antrian').first()
    ...
    if (noRM) {
      ...
      const dataBooking = BookingRegistrasi.query().where('no_rkm_medis', noRM).where('tanggal_periksa', tglPeriksa).where('kd_dokter', kodeDokter).first()
      ...
      if (isRegistered) {
        ...
        try {
          ...
          if (bookingTable.$isPersisted) { //jika berhasil masuk database
  ...
  public async pasienBaru({request, response }: HttpContextContract) {
    ...
    const umur = ()=>{
        var diff_ms = Date.now() - tglLahir.getTime();
