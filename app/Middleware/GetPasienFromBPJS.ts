  ...
  public async handle({ request, response }: HttpContextContract, next: () => Promise<void>) {
    ...
    try {
      ...
      const pasienFromDB = await Pasien.query()
        ...
        .where('nm_pasien', 'like', `%${req.nama_pasien}%`)
        .orWhere((query) => {
          query.where('nm_pasien', 'like', `%${req.nama_pasien}%`).where('tgl_lahir', req.tgl_lahir)
        })
        .first()
      ...
      // await axios
      //   .get(process.env.VCLAIM_ENDPOINT + req.noKtp + '/tglSEP/' + req.tgl_daftar, {
      //     headers: {
      ...
      //       //JIKA PESERTA DITEMUKAN DI APIBPJS
      //       const resData = {
      ...
      //           query
      //             .where('nm_pasien', 'like', `%${resData.nama_pasien}%`)
      //             .where('tgl_lahir', resData.tgl_lahir)
      //         })
      //         .first()
      ...
      //       const pasienFromDB = await Pasien.query().where('no_ktp', req.noKtp).first()
      //       dataToReturn = pasienFromDB || {}
      //     }
      //   })
      //   .catch(async (err) => {
